...
class DinoVisionTransformer(nn.Module):
    ...
    def prepare_tokens_with_masks(self, x, masks=None, marker_ids=None):
        ...
        # 检查self.marker_embed的类型并正确处理
        if isinstance(self.marker_embed, np.ndarray):
            marker_embed = torch.tensor(self.marker_embed).float().unsqueeze(0).to(device=x.device, dtype=x.dtype)
        elif isinstance(self.marker_embed, torch.Tensor):
            marker_embed = self.marker_embed.float().unsqueeze(0).to(device=x.device, dtype=x.dtype)
        else:
            # 如果既不是numpy数组也不是tensor，尝试转换
            marker_embed = torch.tensor(self.marker_embed, dtype=torch.float32).unsqueeze(0).to(device=x.device, dtype=x.dtype)
        ...
        if masks is not None:
            # 确保masks的维度与x匹配
            # x的形状是 [B, num_patches * num_marker, embed_dim]
            # masks的形状是 [B, num_marker, w, h]
            # 需要将masks重塑为与x匹配的形状
            B, num_marker, w, h = masks.shape
            # 首先将masks展平为 [B, num_marker, w*h]
            masks_flat = masks.view(B, num_marker, w * h)
            # 然后重复每个marker的mask到对应的patches
            # 我们需要将每个marker的mask重复num_patches次
            masks_expanded = masks_flat.unsqueeze(2).expand(-1, -1, num_patches, -1)  # [B, num_marker, num_patches, w*h]
            masks_reshaped = masks_expanded.reshape(B, num_marker * num_patches, w * h)  # [B, num_marker * num_patches, w*h]
            # 现在我们需要对每个patch应用mask，所以取平均值或者第一个值
            masks_reshaped = masks_reshaped[:, :, 0]  # 只取第一个值作为mask，去掉最后一个维度
            masks_reshaped = masks_reshaped.unsqueeze(-1)  # [B, num_patches * num_marker, 1]
            
            # 调试信息
            print(f"Debug - x.shape: {x.shape}, masks_reshaped.shape: {masks_reshaped.shape}")
            print(f"Debug - num_patches: {num_patches}, num_marker: {num_marker}")
            
            # 确保维度匹配
            if masks_reshaped.shape[1] != x.shape[1]:
                # 如果维度不匹配，调整masks_reshaped
                if masks_reshaped.shape[1] < x.shape[1]:
                    # 重复masks_reshaped到正确的长度
                    repeat_times = x.shape[1] // masks_reshaped.shape[1]
                    masks_reshaped = masks_reshaped.repeat(1, repeat_times, 1)
                    if masks_reshaped.shape[1] < x.shape[1]:
                        # 如果还不够，再重复一次
                        remaining = x.shape[1] - masks_reshaped.shape[1]
                        masks_reshaped = torch.cat([masks_reshaped, masks_reshaped[:, :remaining, :]], dim=1)
                else:
                    # 截断masks_reshaped
                    masks_reshaped = masks_reshaped[:, :x.shape[1], :]
            
            x = torch.where(masks_reshaped, self.mask_token.to(x.dtype).unsqueeze(0), x)
